...
1. **DOCX 智能再排版**  
   - 加载现有 Word 文档，对段落类型进行识别与调整（标题、正文、代码、表格、公式等）
   - 右键点击预览区段落可修改类型，一键应用样式设置重新生成 DOCX
   - 支持编号列表段落的字体同步修改（包括编号本身）
...
3. **Markdown(.md) → DOCX**  
   - 自动解析 Markdown 语法：`#`~`######` 标题、代码块、引用、公式等
   - 预览区显示每段的识别类型，支持右键修改
   - 按标题层级、正文、列表等结构生成 DOCX，复用同一套样式设置和模板
